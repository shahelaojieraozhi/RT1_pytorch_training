...
# The following will create a trajectories of length 6.
trajectory_length = 6
...
episode_sec_len = 20  # episode section length
for idx in tqdm(range(700), file=sys.stdout):

  rt1_episodic_dataset = rt1_builder.as_dataset(split=f'train[{idx*episode_sec_len}:{(idx+1)*episode_sec_len}]')
  ...
  torch.save(output_data, f'./VLMART/rt1_whole_data/{idx*episode_sec_len}-{(idx+1)*episode_sec_len}_episode.h5')

print(f"{datetime.datetime.now()}")

# 100 个 episode 内存大小： 6.12 个G
# 20个 episode 内存大小：1.2 G
